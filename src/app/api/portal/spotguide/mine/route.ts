...
export async function GET(request: NextRequest) {
  ...
  const [{ data: sr }, { data: sv }, { data: dr }] = await Promise.all([
    spotIds.length ? db.from("spot_ratings").select("spot_id, ratings").eq("contact_id", user.contactId).in("spot_id", spotIds) : Promise.resolve({ data: [] }),
  ...
  const spots: Record<string, { ratings?: Record<string, number>; model?: string }> = {};
  for (const r of sr ?? []) spots[r.spot_id] = { ...(spots[r.spot_id] ?? {}), ratings: r.ratings ?? {} };
