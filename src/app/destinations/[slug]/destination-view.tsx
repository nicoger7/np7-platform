...
export type DestinationData = { destination: Destination; trips: Trip[]; hotels: Hotel[]; centers: Center[] };

/**
 * `draft: true` is the team preview (/destinations/<slug>/preview): it reads
 * the destination through the service role and whatever its status, so a
 * draft can be looked at before anyone publishes it. The public page never
 * passes it, and keeps reading exactly what the public may read.
 */
export async function getDestination(slug: string, opts: { draft?: boolean } = {}): Promise<DestinationData | null> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const sb = supabase as any;
  const { data: destination } = opts.draft
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ? await (createAdminClient() as any).from("destinations").select("*").eq("slug", slug).maybeSingle()
    : await sb.from("destinations").select("*").eq("slug", slug).eq("status", "published").maybeSingle();
...
/**
 * The whole destination page below the access check. Shared by the public page
 * (ISR, published only) and the team preview (dynamic, drafts included), so a
 * preview is exactly what publishing will show.
 */
export function DestinationView({ res, preview = false }: { res: DestinationData; preview?: boolean }) {
          ...
          <section id="conditions" className="scroll-mt-[120px] relative bg-[#00374a] text-white py-20 sm:py-28 overflow-hidden">
            ...
            <div className="relative max-w-[1100px] mx-auto px-6 sm:px-8">
              ...
              <Reveal>
                ...
                /* No chart to carry the section (no stats, or the model is
                   blind to the local wind, as at Garda and Volosko), so the
                   facts carry it: the two that decide a trip as big tiles,
                   the rest beneath. A row of pills alone read as unfinished. */
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 sm:gap-4">
                  {facts.map((f, i) => {
                    const big = i < 2;
                    const rest = facts.length - 2;
                    const span = big
                      ? (facts.length === 1 ? "sm:col-span-2 lg:col-span-6" : "lg:col-span-3")
                      : rest === 1 ? "sm:col-span-2 lg:col-span-6" : rest === 2 ? "lg:col-span-3" : "lg:col-span-2";
                    return (
                      <Reveal key={f.label} delay={i * 60} className={span}>
                        <div className={`h-full rounded-2xl border border-white/10 ${big ? "bg-white/[0.08] p-6 sm:p-7" : "bg-white/[0.05] p-5"} flex items-start gap-4`}>
                          <span className={`shrink-0 rounded-full bg-[#ffc42e]/15 text-[#ffc42e] flex items-center justify-center ${big ? "w-12 h-12" : "w-10 h-10"}`}>
                            <StatIcon label={f.label} className={big ? "w-6 h-6" : "w-5 h-5"} />
                          </span>
                          <span className="min-w-0">
                            <span className="block text-[10.5px] font-bold tracking-[0.16em] uppercase text-[#ffc42e]/85 mb-1.5">{f.label}</span>
                            <span className={`block font-bold text-white leading-snug [text-wrap:balance] ${big ? "text-[20px] sm:text-[24px] tracking-[-0.01em]" : "text-[15.5px]"}`}>{f.value}</span>
                          </span>
                        </div>
                      </Reveal>
                    );
