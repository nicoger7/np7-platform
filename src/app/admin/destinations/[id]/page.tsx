...
type Partner = { name: string; description: string; url: string };
interface Dest {
...
export default function DestinationEditor({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [picker, setPicker] = useState<"hero" | "gallery" | null>(null);
    ...
    <div className="max-w-[760px]">
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          ) : <button onClick={() => setPicker("hero")} className={`${inputClass} text-left admin-muted max-w-[200px]`}>Pick hero image…</button>}
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            <button onClick={() => set("partners", [...partners, { name: "", description: "", url: "" }])} className="text-xs text-[#0aa3c7] hover:underline">+ Add partner</button>
          </div>
        </div>
      ...
      {picker && (
        <ImagePickerModal
          defaultFolder={folder ? `${folder}/${picker}` : undefined}
          onSelect={(url) => {
