...
export default function EmailEditorPage() {
  // /admin/emails/<template_key> — the email this page edits. The overview at
  // /admin/emails is the only way in, so there is no list here: the card grid
  // replaced the table it used to show.
  const routeKey = String(useParams()?.key ?? "");
  const automation = AUTOMATIONS.find((a) => a.key === routeKey);
  ...
  // The route key IS the email: /admin/emails/<template_key>.
  useEffect(() => {
    if (autoOpenedRef.current || loading) return;
    autoOpenedRef.current = true;
    if (!routeKey) return;
    const t = templates.find((x) => x.template_key === routeKey);
    if (t) { startEdit(t); return; }
    // Most automations have never been customised, so they have no row. Start
    // one from the built-in wording rather than dumping you on an empty list —
    // that dead end was the old "click an email, land nowhere" bug.
    skipDirty.current = true;
    setShowNew(true); setEditId(null); setShowAdvanced(false);
    setForm({ ...EMPTY_FORM, name: automation?.name ?? routeKey, template_key: routeKey,
      subject_line: DEFAULT_SUBJECTS[routeKey] || "", body: DEFAULT_BODIES[routeKey] || "" });
  }, [templates, loading]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          <Link href="/admin/emails" className="inline-flex items-center gap-1.5 text-xs font-semibold admin-muted hover:text-[var(--admin-accent)] transition-colors mb-1">
          ...
          <h1 className="text-2xl font-bold admin-heading mb-1">{automation?.name ?? "Email"}</h1>
          <p className="text-sm admin-muted">{automation?.trigger ?? routeKey}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="lg:w-60 shrink-0 flex lg:flex-col gap-1.5 lg:max-h-[82vh] lg:overflow-y-auto lg:pr-1">
          <Link href="/admin/emails" className="shrink-0 mb-1 flex items-center gap-1.5 text-xs font-semibold admin-muted hover:text-[var(--admin-accent)] transition-colors">
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6" /></svg>
            All emails
          </Link>
          {sorted.map((t) => {
