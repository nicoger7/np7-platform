    ...
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { setShowNew(!showNew); setEditId(null); setForm({ name: "", status: "open", priority: "medium", due_date: "", assignee: "", experience_id: "", notes: "" }); }} className="px-4 py-2 bg-[var(--admin-accent)] hover:bg-[var(--admin-accent)]/90 text-[var(--admin-accent-contrast)] text-sm font-bold rounded-lg transition-colors">
            New To-Do
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-5">
        {["", ...STATUSES].map((s) => (
          <button key={s} onClick={() => setFilterStatus(s)} className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors capitalize ${filterStatus === s ? "bg-[var(--admin-accent)]/15 text-[#0aa3c7]" : "admin-surface admin-muted"}`} style={{ border: "1px solid var(--admin-border)" }}>
            {s || "All"}
          </button>
        ))}
      </div>
