...
/** The next bookable weeks, for the member home's "book your next trip" strip.
 *  Public-site rules apply: published + on-website experiences, weeks that
 *  haven't started, cheapest sellable package as the "from" price. */
export type UpcomingTrip = {
  slug: string; title: string; location: string | null; hero: string | null;
  dateStart: string; dateEnd: string | null; fromPrice: number | null;
};
export async function getUpcomingTrips(limit = 3): Promise<UpcomingTrip[]> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const db = createAdminClient() as any;
  const today = new Date().toISOString().slice(0, 10);
  const { data } = await db
    .from("exp_experiences")
    .select("slug,title,location,hero_image,website_visible,status,exp_editions(id,date_start,date_end,status),exp_packages(price,status,website_visible,edition_id)")
    .eq("status", "published");
  type Ed = { id: string; date_start: string | null; date_end: string | null; status: string | null };
  type Pkg = { price: number | null; status: string | null; website_visible: boolean | null; edition_id: string | null };
  const trips: UpcomingTrip[] = [];
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  for (const e of (data ?? []) as any[]) {
    if (e.website_visible === false) continue;
    const next = ((e.exp_editions ?? []) as Ed[])
      .filter((ed) => ed.status === "published" && ed.date_start && ed.date_start > today)
      .sort((a, b) => (a.date_start! < b.date_start! ? -1 : 1))[0];
    if (!next) continue;
    const sellable = ((e.exp_packages ?? []) as Pkg[])
      .filter((p) => p.status === "active" && p.website_visible !== false && p.price != null);
    const scoped = sellable.filter((p) => p.edition_id === next.id);
    const prices = (scoped.length ? scoped : sellable).map((p) => Number(p.price));
    trips.push({
      slug: e.slug, title: e.title, location: e.location ?? null, hero: e.hero_image ?? null,
      dateStart: next.date_start!, dateEnd: next.date_end ?? null,
      fromPrice: prices.length ? Math.min(...prices) : null,
    });
  }
  return trips.sort((a, b) => (a.dateStart < b.dateStart ? -1 : 1)).slice(0, limit);
}
