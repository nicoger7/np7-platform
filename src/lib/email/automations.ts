...
export const AUTOMATIONS: Automation[] = [
  ...
  { key: "voucher_expiry_reminder", name: "Gift voucher — expiry reminder", stage: "Vouchers", trigger: "~30 and ~7 days before a voucher's redeem-by date (daily cron)", division: "experience", kind: "lifecycle", source: "scheduled" },
  ...
  { key: "event_date_confirmed_balance", name: "Event date confirmed — balance due", stage: "Events", trigger: "When you confirm an event date — payment link to everyone who can make it", division: "experience", kind: "transactional", source: "staff" },
  { key: "event_date_not_running", name: "Event date not running — refund", stage: "Events", trigger: "When you confirm an event date — refund notice to standbys whose chosen dates didn't run", division: "experience", kind: "transactional", source: "staff" },
  ...
  { key: "post_trip_thank_you", name: "Thank you + review", stage: "Post-trip", trigger: "1 day after the trip ends", division: "experience", kind: "lifecycle" , source: "scheduled" },
  ...
  { key: "credit_note_sent", name: "Storno / credit note sent (manual)", stage: "Billing", trigger: "When you email a Storno or credit note from a booking's Documents tab or the Invoices page (PDF attached). Says what was reversed and whether a refund follows; never asks for payment", division: "experience", kind: "transactional", source: "staff" },
  ...
  { key: "transfer_instructions", name: "Bank transfer — the account details", stage: "Billing", trigger: "The moment a member starts a bank transfer from their trip page and Stripe issues them an IBAN. Carries the account, the reference and the exact amount, so their only copy is not the browser tab they are looking at", division: "experience", kind: "transactional", source: "guest" },
  { key: "transfer_failed", name: "Bank transfer didn't go through", stage: "Billing", trigger: "When Stripe reports the transfer failed. The guest believes they paid and their spot is not held, so this must never be silent", division: "experience", kind: "transactional", source: "guest" },
