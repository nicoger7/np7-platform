...
import { useAdminEnv, type AdminEnv } from "@/app/admin/env-context";
 ...
 *
 * `world` is passed by the route, so /admin/performance/finance can only ever
 * be NP7 Performance's budget and the address says so. Without it the page
 * falls back to whichever world the shell is in, which is what the old
 * /admin/finance did and why it read like a group-wide page it never was.
 */
export function Budget({ world }: { world?: AdminEnv }) {
  ...
  async function createPlan(copyFrom?: string) {
    ...
    if (!entity) return;
    // A variant nobody named is a variant nobody meant to make.
    let name: string | undefined;
    if (copyFrom) {
      const answer = prompt("What are you trying out?\n\ne.g. \"Order everything in January\" or \"350 boards at 900\"");
    ...
    const res = await fetch("/api/admin/finance/plans", {
      ...
      body: JSON.stringify({ entity_id: entity.id, year, copy_from: copyFrom, name }),
    });
    setBusy(false);
    ...
    <div className="fin p-4 sm:p-6 space-y-5">
      ...
      <div className="fin-card !py-2.5 !px-3 flex flex-wrap items-center gap-x-3 gap-y-2">
        ...
        {filterObjects.length > 0 && (
          <>
            <span className="w-px self-stretch" style={{ background: "var(--fin-hairline)" }} />
            <ScopePicker objects={filterObjects} value={objectId}
                         onChange={(id) => setObjectPick(id ? { world: env, id } : null)} />
          </>
        )}
        ...
        {plans.length > 1 && (
          <>
            <span className="w-px self-stretch" style={{ background: "var(--fin-hairline)" }} />
            {/* One plan is the one you are working to. The rest are things you
                were trying out, and they should never look equal to it. */}
            <div className="fin-seg">
              {plans.map((p) => (
                <button key={p.id} data-on={selectedPlan === p.id}
                        title={p.status === "active" ? "The plan in force" : "A variant. Not what you are working to."}
                        onClick={() => setPlanPick({ world: env, id: p.id })}>
                  {p.status === "active" ? "Master" : p.name}
                </button>
              ))}
            </div>
          </>
        ...
        {board?.plan && board.plan.status !== "active" && (
          <span className="fin-sub" style={{ color: "var(--viz-neg)" }}>
            Looking at a variant, not the master.
          </span>
        )}
