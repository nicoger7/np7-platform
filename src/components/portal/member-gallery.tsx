  ...
  const thumb = (src: string, idx: number) => {
    ...
    return (
      <div key={idx} className="relative group block w-full aspect-square overflow-hidden rounded-xl bg-[#eef3f4] ring-1 ring-black/[0.04]">
        ...
        {/* Share CTA leads the gallery — below the review nudge, above the keepers
          banner (founder-ordered). Only when there's a shareable photo (own or
          the shared "everyone" memories). */}
      {sharePhotos.length > 0 && (
        <div className="mb-4">
          <button type="button" onClick={() => setShare(sharePhotos[0])}
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 rounded-full text-[14.5px] font-black text-white shadow-[0_10px_26px_rgba(240,123,32,0.26)] hover:-translate-y-0.5 transition-transform"
