...
type Mine = { id: string; component_id: string | null; label: string; price: number | null; status?: string | null };
...
const STEPS = [
  ...
  { t: "Request extra nights", d: "Want to arrive early or stay longer? Request below." },
...
export function TripAddons({ bookingId, depositPaid }: { bookingId: string; depositPaid: boolean }) {
  ...
  const requestedIds = new Set(mine.map((m) => m.component_id).filter(Boolean));
    ...
    <div className="space-y-5">
      ...
      {mine.length > 0 && (
        <div className="border-t border-[#f3ede2] pt-4 space-y-2">
          {mine.map((m) => {
            const confirmed = (m.status ?? "confirmed") === "confirmed";
            return (
      ...
      {!loading && offer.length > 0 && (
        <div className="border-t border-[#f3ede2] pt-4">
          ...
          <p className="text-[12px] text-[#9aa6ac] mt-2.5">Requests aren&apos;t charged automatically — we confirm availability first, then add it to your balance.</p>
        </div>
      )}
    </div>
  );
