...
const DISCIPLINES = [
  {
    ...
    body: [
      ...
      "It teaches you patience, how to read nature, how to stay calm when it turns wild. Those lessons follow you well off the water. To us it is simply the best sport in the world, and every NP7 week is built around it.",
    ],
  },
  {
    ...
    body: [
      ...
      "It opens the wind range wide: the light days that used to keep you on the beach become some of your best sessions. If you love the fine feeling of dialling something in, windfoiling becomes the next obsession. We coach it right alongside your windsurfing all week.",
    ],
  },
  {
        ...
        {DISCIPLINES.map((d) => {
          ...
          return (
            <button key={d.key} type="button" onClick={() => setActive(d.key)} aria-expanded={on}
              ...
              {/* Zoom wrapper. Safari (WebKit) does NOT clip force-composited
                  descendants by an ancestor's border-radius OR clip-path — so the
                  inner image must not carry transform-gpu, and the radial-gradient
                  mask forces WebKit to rasterize the clip onto every descendant
